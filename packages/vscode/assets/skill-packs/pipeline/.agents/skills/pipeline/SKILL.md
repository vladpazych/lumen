...
- Export exactly `config` (plain dict) and `generate` (async function)
...
- If `generate()` reads a Modal secret from `os.environ`, declare `serve_secrets = ["secret-name"]` in the module so the server attaches it
