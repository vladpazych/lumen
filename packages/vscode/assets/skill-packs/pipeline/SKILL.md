---
name: pipeline
description: "Create or edit pipeline files in server/pipelines/. Use when adding a pipeline, wiring inference, defining params, or integrating Modal GPU logic."
---

# Pipeline Authoring

A Lumen pipeline is one Python module in `server/pipelines/` that exports `config` and `generate`.
...
- Keep pipeline-specific logic in the pipeline module. Shared runtime logic belongs in `server/lumen_server/`.
