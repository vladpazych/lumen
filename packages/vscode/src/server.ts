...
/** Resolves lumen.serverPath with ${workspaceFolder} expansion. Returns "" if unset. */
export function resolveServerPath(): string {
  const raw = vscode.workspace.getConfiguration("lumen").get<string>("serverPath", "")
  if (!raw) return ""
...
export class ServerManager {
  ...
  start(): void {
    const serverPath = resolveServerPath()
    if (!serverPath) {
      vscode.window.showErrorMessage("Set lumen.serverPath first")
      return
    }
