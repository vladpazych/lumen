...
- The framework mounts `.authkey` into the Modal container.
...
The VS Code extension manages the dev server and hot-reloads automatically. You do NOT run the server yourself — just write pipelines and validate.

```sh
uv run pytest        # validate schema + contract compliance
