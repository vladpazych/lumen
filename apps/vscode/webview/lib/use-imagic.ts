...
import type { DevServerState, ImagicConfig, PipelineConfig, ServerStatus } from "../../shared/types"
...
type Action =
  ...
  | { type: "configsUpdated"; configs: ImagicConfig[] }
  ...
  | { type: "addConfig"; config: ImagicConfig }
...
export function useImagic() {
  ...
  const addConfig = useCallback(
    (service: string, pipeline: string, schemas: Record<string, PipelineConfig[]>, existingConfigs: ImagicConfig[]) => {
      ...
      const config: ImagicConfig = { id: crypto.randomUUID(), name, service, pipeline, params: {} }
