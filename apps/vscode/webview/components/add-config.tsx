...
export function AddConfig({ schemas, serverStatuses, onAdd, onCancel }: Props) {
    ...
    <div className="border border-border rounded-md">
      <Inset spacing="normal">
        <Stack spacing="normal">
          <Select value={service ?? undefined} onValueChange={handleServiceChange}>
            <SelectTrigger>
              ...
              {serviceUrls.map((url) => (
                <SelectItem key={url} value={url}>
                  <Row spacing="snug" align="center">
                    <StatusDot variant={statusVariant[serverStatuses[url] ?? "disconnected"]} size="xs" />
                    {shortenUrl(url)}
                  </Row>
                </SelectItem>
              ))}
          ...
          <Row spacing="snug" justify="end">
            ...
            <Button variant="accent" size="sm" disabled={!service || !pipeline} onClick={handleAdd}>
              Add
            </Button>
          </Row>
        </Stack>
