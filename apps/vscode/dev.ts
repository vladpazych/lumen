...
console.log("[imagic-dev] initial build...")
...
function reinstall() {
  try {
    execSync("bunx @vscode/vsce package --no-dependencies -o dist/imagic-vscode.vsix", { stdio: "pipe" })
    execSync("code --install-extension dist/imagic-vscode.vsix", { stdio: "pipe" })
    console.log("[imagic-dev] reinstalled — reload window to pick up changes")
  } catch (e) {
    console.error("[imagic-dev] reinstall failed:", e instanceof Error ? e.message : e)
  }
}
