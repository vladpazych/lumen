...
- **Dynamic** — HTTP server exposing `GET /pipelines`, `GET /pipelines/:id`, `POST /pipelines/:id/generate`, `GET /pipelines/:id/runs/:runId`. modal-pipelines is one implementation. Add URL to `imagic.serverUrls` setting — no extension code needed.
...
```json
[
  { "id": "a1b2c3", "service": "http://localhost:8000", "pipeline": "txt2img", "params": { "prompt": "a cat" } },
  {
    "id": "d4e5f6",
