...
import type { ImagicConfig, PipelineConfig, ServerStatus } from "../shared/types"
...
const log = vscode.window.createOutputChannel("Imagic")

const HEALTH_POLL_MS = 10_000

export class ImagicEditorProvider implements vscode.CustomTextEditorProvider {
  private static readonly viewType = "imagic.stateViewer"
  ...
  static register(provider: ImagicEditorProvider): vscode.Disposable {
    return vscode.window.registerCustomEditorProvider(ImagicEditorProvider.viewType, provider, {
  ...
  private getDevServerUrl(): string | null {
    ...
    const url = vscode.workspace.getConfiguration("imagic").get<string>("devServerUrl", "")
    return path && url ? url : null
  }

  private getAllServerUrls(): string[] {
    const devUrl = this.getDevServerUrl()
    const deployed = vscode.workspace.getConfiguration("imagic").get<string[]>("serverUrls", [])
  ...
  async resolveCustomTextEditor(document: vscode.TextDocument, webviewPanel: vscode.WebviewPanel): Promise<void> {
    ...
    const configListener = vscode.workspace.onDidChangeConfiguration((event) => {
      if (
        event.affectsConfiguration("imagic.serverUrls") ||
        event.affectsConfiguration("imagic.devServerUrl") ||
        event.affectsConfiguration("imagic.serverPath")
      ) {
        for (const url of this.getAllServerUrls()) {
          if (!this.schemas[url]) {
            this.refreshSchemas(url)
  ...
  private async doGenerate(
    ...
    if (serverUrl === FAL_PROVIDER_URL) {
      if (!this.falApiKey) throw new Error("fal.ai API key not set. Run 'Imagic: Set fal.ai API Key'.")
  ...
  private ensureIds(configs: ImagicConfig[]): boolean {
    let assigned = false
    for (const config of configs) {
      if (!config.id) {
  ...
  private parseDocument(document: vscode.TextDocument): ImagicConfig[] {
    const text = document.getText().trim()
    if (!text || text === "[]" || text === "{}") return []
    try {
      const parsed = JSON.parse(text) as unknown
      if (Array.isArray(parsed)) return parsed as ImagicConfig[]
  ...
  private migrateOldFormat(raw: Record<string, unknown>): ImagicConfig[] {
    const configs: ImagicConfig[] = []
  ...
  private async writeDocument(document: vscode.TextDocument, configs: ImagicConfig[]): Promise<void> {
  ...
  private async saveAsset(
    ...
    const base = basename(documentUri.fsPath, ".imagic")
    const now = new Date()
